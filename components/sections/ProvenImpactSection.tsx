  ...
  return (
    <section className="relative w-full bg-white overflow-hidden">
      ...
      <div className="relative max-w-[1440px] mx-auto px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-12 sm:py-16 md:py-20 lg:py-24 xl:py-[100px] flex flex-col lg:flex-row items-center lg:items-center justify-end gap-8 md:gap-12 lg:gap-[164px]">
        ...
        <div className="hidden lg:flex lg:flex-1 lg:h-[516px] relative overflow-hidden">
          {/* Top fade gradient */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-[436px] h-[30px] sm:h-[50px] md:h-[80px] lg:h-[154px] pointer-events-none z-10">
            <div
          ...
          <motion.div
            className="flex flex-col gap-6 sm:gap-8 md:gap-12 lg:gap-[80px]"
            animate={{
          ...
          >
            ...
            {duplicatedStats.map((stat, index) => (
              <div key={index} className="flex flex-col gap-1 w-full">
                ...
                <p className="text-[64px] sm:text-[80px] md:text-[100px] lg:text-[150px] font-bold leading-none tracking-[2px] outlined-stat-number">
                  {stat.number}
                </p>
          ...
          <div
            className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[436px] h-[30px] sm:h-[50px] md:h-[80px] lg:h-[164px] pointer-events-none z-10"
            style={{
              background: 'linear-gradient(to bottom, rgba(255,255,255,0), white)',
            }}
          />
        ...
        <div className="order-1 lg:order-2 w-full lg:w-[648px] shrink-0 flex flex-col gap-6 sm:gap-7 md:gap-8 items-start lg:items-end">
          ...
          {/* Mobile/tablet stats — hidden on lg+ (desktop uses the left column) */}
          <div className="lg:hidden w-full h-[120px] sm:h-[150px] md:h-[190px] max-h-[120px] sm:max-h-[150px] md:max-h-[190px] relative overflow-hidden">
            {/* Top fade */}
            <div className="absolute top-0 left-0 w-full h-[30px] sm:h-[50px] pointer-events-none z-10"
              style={{ background: 'linear-gradient(to bottom, white, rgba(255,255,255,0))' }}
            />
            <motion.div
              className="flex flex-col gap-6 sm:gap-8 md:gap-12"
              animate={{ y: [0, -(stats.length * (94 + 24))] }}
              transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}
            >
              {duplicatedStats.map((stat, index) => (
                <div key={index} className="flex flex-col gap-1 w-full">
                  <div className="flex items-center gap-2">
                    <Image src={stat.icon} alt="" width={15} height={12} className="object-contain w-[13px] h-[10px]" />
                    <p className="text-[10px] font-normal text-[#222222] tracking-[0.8px] uppercase leading-[15px]">{stat.label}</p>
                  </div>
                  <p className="text-[64px] sm:text-[80px] md:text-[100px] font-bold leading-none tracking-[2px] outlined-stat-number">{stat.number}</p>
                  <p className="text-[10px] font-normal text-[#444444] tracking-[0.4px] leading-[15px]">{stat.description}</p>
                </div>
              ))}
            </motion.div>
            {/* Bottom fade */}
            <div className="absolute bottom-0 left-0 w-full h-[30px] sm:h-[50px] pointer-events-none z-10"
              style={{ background: 'linear-gradient(to bottom, rgba(255,255,255,0), white)' }}
            />
          </div>

          {/* Less spend block */}
          <div className="text-[24px] sm:text-[28px] md:text-[34px] lg:text-[40px] xl:text-[44px] font-bold text-[#ed6b52] leading-[1.2] tracking-[-1.5px] md:tracking-[-2px] w-full text-left lg:text-right">
